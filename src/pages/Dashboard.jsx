...
const Dashboard = () => {
  ...
  const [emotionStats, setEmotionStats] = useState([
    ...
    { name: "Cinta", code: 2, value: 0, color: "#FF9694" },
  ...
  useEffect(() => {
    const fetchResponseData = async () => {
      try {
        ...
        const calculateTotalSum = newEmotionStats.reduce((sum, item) => sum + item.value, 0);
        setEmotionStats(newEmotionStats);
        setTotalSum(calculateTotalSum);

        setEmotionData(response.data);
        setLatestEmotion(emotionTranslator(response.data[0].emotion_code));
      } catch (error) {
        console.error("Error fetching data:", error);
      } finally {
        setIsLoading(false);
  ...
  return (
    <div className="app-container">
      <main className="main-content dashboard-content">
        ...
        <div className="welcome-layout">
          ...
          <div className="mood-section">
            ...
            <p className="mood-paragraph">
              {(!latestEmotion) ? <p>Keliatannya kamu belum pernah curhat sebelumnya. Cobalah fitur <Link to="/curhat"><span className="highlight-name" style={{fontWeight: 'bold'}}>curhat</span></Link> kami!</p> : <p>Jejak perasaanmu terakhir kali itu <span className="highlight-name" style={{fontWeight: 'bold'}}>{latestEmotion.emotion.toLowerCase()}</span>, bagaimana dengan hari ini?</p>}
        ...
        {isLoading ? (<Loading />) : emotionData.length === 0 ? <p>Anda belum pernah melakukan sesi curhat!</p> : (
          <>
